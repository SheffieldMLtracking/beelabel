...
def getcode(save, im, debug=False, shift=None, thresholds=[1.5, 0.6]):
    ...
    try:
        ...
        postcoords = [startx + p * xstep * 0.1, starty + p * ystep * 0.1, startx + (p + step_length * 11) * xstep * 0.1,
                      starty + (p + step_length * 11) * ystep * 0.1]
    ...
    if (binaryvals[2] != 0) or (binaryvals[3] != 1) or (binaryvals[4] != 0) or (binaryvals[5] != 1) or (
            binaryvals[6] != 1) or (binaryvals[7] != 0):
        if debug:
...
class Align():
        ...
        for cam, image_fns in self.image_filenames.items():
            ...
            for fn in image_fns:
                ...
                if contact is not None:
                    for con in contact:
                        if con['prediction'] < 8:
                            self.coords[cam].append(
                                {'triggertime': photo_list[-1]['record']['triggertime'], 'xy': [con['x'], con['y']],
                                 'score': con['prediction']})
            ...
            for gc in self.grouped_coords[trigtime]:
                ...
                if nest_item is not None:
                    if gc['cam'] in self.newitems[nest_item]['imgcoords']:
                        ...
                        if (np.linalg.norm(np.array(gc['imgxy']) - np.array(nestxy)[None, :],
                                           axis=1)) < nest_ignore_radius: continue
    ...
    def process_coords(self):
        ...
        for cam in self.coords:
            for j, c in enumerate(self.coords[cam]):
                ...
                self.grouped_coords[c['triggertime']].append(
                    {'origin': origin, 'vect': vect, 'score': c['score'], 'imgxy': c['xy'], 'cam': cam})
    ...
    def get_markers(self, image, threshold, debug=False):
        ...
        for step in range(3, 9):
            inittempl[int(step * 100):int(step * 100 + 100), int(150 - halfwidth):int(150 + halfwidth)] = 255.0 - (msg[
                                                                                                                       step] * 255.0)  # int(step*slope+scale*0.5):int(scale*1.5+step*slope)] = 255.0-(msg[step]*255.0)
            initmask[int(step * 100 + maskborderh):int(step * 100 + 100 - maskborderh),
            int(150 - halfwidth + maskborderw):int(150 + halfwidth - maskborderw)] = 1
        inittempl[int(0 * 100 + 50):int(0 * 100 + 100), int(150 - halfwidth):int(
            150 + halfwidth)] = 220.0  # int(step*slope+scale*0.5):int(scale*1.5+step*slope)] = 255.0-(msg[step]*255.0)
        ...
        for n, scale in enumerate(np.linspace(0.05, 0.2, 15)):  # (0.135,0.135,1):
            ...
            for angle in np.linspace(-10, 10, 20):
                ...
                for pt in zip(*loc[::-1]):
                    ...
                    data = {'loc': pt, 'coord': shifted_pt, 'scale': scale * 100, 'angle': angle,
                            'score': res[pt[1], pt[0]], 'w': w, 'h': h}
    ...
    def find_markers(self, refresh_cache=False, debug=False, threshold=0.91, photo_indicies=None):
        ...
        if not hasattr(self, 'found'):
            self.found = {}
        fnd = {}
        self.shift = int(next(iter(self.cam_photos.values())).shape[
                             0] / 3)  # this is for debugging, might change value - not cached...
    ...
    def draw_found(self, cam, imthresh=30, draw_3d_location=False):
        ...
        for f in found_list:
            ...
            plt.plot([fnd['postcoords'][0], fnd['postcoords'][2]],
                     np.array([fnd['postcoords'][1], fnd['postcoords'][3]]), 'y-')
            plt.plot([fnd['postcoords'][0], fnd['postcoords'][2]],
                     np.array([fnd['postcoords'][1], fnd['postcoords'][3]]), 'y+', markersize=10, mew=1)
    ...
    def get_pixel_loc(self, cam, markercoords):
        ...
        if len(pvec.shape) == 1:
            pvec = pvec[None, :]
        res = np.array([1024 + 1024 * (-pvec[:, 1] / pvec[:, 0]) / self.hfovw,
                        (1024 + 1024 * (pvec[:, 2] / pvec[:, 0] / self.vfovw)) * 0.75]).T
    ...
    def build_newitems(self):
                ...
                if (len(it) == 3):  # marker (not _top or _bottom, we deal with these next).
                    ...
                    for cam in self.found:
                        for f in self.found[cam]:
                            ...
                            if it == markerid:
                                self.newitems[it + '_top']['imgcoords'][cam] = deepcopy(
                                    self.found[cam][f]['postcoords'][:2])
                                self.newitems[it + '_bottom']['imgcoords'][cam] = deepcopy(
                                    self.found[cam][f]['postcoords'][2:])
            else:
        ...
        for it in self.items:
            if len(re.findall('m[0-9]{2}_', it)) > 0:  # is this landmark m??_something?
                ...
                if 'imgcoords' in self.items[it]: self.newitems[it]['imgcoords'] = {**self.items[it]['imgcoords'],
                                                                                    **self.newitems[it]['imgcoords']}
    ...
    def generate_alignment(self, maxiters=1000, refresh_cache=False, printlabels=False):
        ...
        hashnumber = gethash(
            (self.config_filename + " " + " ".join([fns[0] for cam, fns in self.image_filenames.items()])).encode(
                "utf-8"))
            ...
            if adjustpos:
                ...
                if n[-6:] == 'bottom':
                    ...
                    if 'bounds' in self.newitems[n]:
                        bounds.append([self.newitems[n]['coords'][0] - self.newitems[n]['bounds'],
                                       self.newitems[n]['coords'][0] + self.newitems[n]['bounds']])
                        boundstrings.append(n + "x")
                        bounds.append([self.newitems[n]['coords'][1] - self.newitems[n]['bounds'],
                                       self.newitems[n]['coords'][1] + self.newitems[n]['bounds']])
                        boundstrings.append(n + "y")
                        bounds.append([-2, 2])  # move up or down by 1.5m
                        boundstrings.append(n + "z")
                    else:
                        bounds.append([self.newitems[n]['coords'][0] - movementallowed,
                                       self.newitems[n]['coords'][0] + movementallowed])
                        boundstrings.append(n + "x")
                        bounds.append([self.newitems[n]['coords'][1] - movementallowed,
                                       self.newitems[n]['coords'][1] + movementallowed])
                        boundstrings.append(n + "y")
                        bounds.append([-2, 2])  # move up or down by 1.5m
                        boundstrings.append(n + "z")
        ...
        opt = optimize.minimize(self.compute_error, np.mean(np.array(bounds), 1), bounds=bounds,
                                options={'maxiter': maxiters}, callback=showprog)
